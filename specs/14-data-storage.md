...
| 根目录 | 路径 | 存什么 |
|--------|------|--------|
| **App Data** | `${app.getPath('userData')}/` | 应用级数据：设置、凭证、会话、记忆 |
| **Workspace** | 用户选择的项目目录 | 项目级数据：Soul 文件、MCP 配置 |

### App Data 目录结构

```
~/.config/pi-desktop/                    # Linux
~/Library/Application Support/pi-desktop/ # macOS
%APPDATA%/pi-desktop/                    # Windows
│
...
- `steps` 顺序稳定，可按时间回放
- `awaiting_confirmation` 不能只存在内存里，至少要能恢复到“待确认”状态
- session 内事件流和 `audit.log` 能通过 `runId` 关联

v1 不强制单独引入 `runs/{runId}.json` 文件，但必须保证 session 数据已经足以恢复关键执行轨迹。
...
| 不进入 git | `.pi/` 下不存凭证；App Data 目录本身不在任何仓库内 |
...
使用 `sqlite-vec`（SQLite 扩展）存储 embedding 向量：
...
CREATE TABLE memories (
  ...
  source TEXT NOT NULL,            -- 来源：session / user_note / auto_extract
  sessionId TEXT,                  -- 关联会话 ID（如果有）
  createdAt TEXT NOT NULL,
  lastAccessedAt TEXT NOT NULL,    -- 最近检索命中时间（用于衰减）
  accessCount INTEGER DEFAULT 0    -- 命中次数
);
...
用户是全栈工程师，正在开发一个 Electron 桌面 Agent 应用。
技术栈：React 19 + TypeScript + Tailwind CSS 4 + HeroUI。
偏好浅色主题，注重视觉品质。
```

原始 Markdown 文件是向量数据库的备份/调试用——可以直接阅读记忆内容，方便用户检查 Agent 记住了什么。
...
当前应用已有 `desktop-shell-state.json` 存储所有会话。迁移到新的多文件方案：

```
应用启动
  → 检测 desktop-shell-state.json 是否包含 sessions 数组
  → 如果包含：
      1. 为每个 session 创建 sessions/{id}.json
      2. 生成 sessions/index.json
      3. 从 desktop-shell-state.json 中移除 sessions 数组
      4. 写入迁移完成标记
  → 如果不包含（已迁移或全新安装）：
      跳过
```
...
| 08-soul-files | Workspace 下 `.pi/` 目录的 Soul 文件 |
| 09-rag-and-embedding | sqlite-vec 的表结构和检索接口 |
